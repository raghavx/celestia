...
- `geo.time` — `TimeshapeTimeZoneResolver` (city sample + ocean fallback),
  `LocalToUtc` (normal / DST gap / DST fold / unknown-time), `tzdb` version read.
...
| US1 historical offsets, pre-1970, standard-offset change (SC-001) | `geo.BirthMomentCorpusTest` (corpus rows) |
...
| US2 lat/lon → zone vs reference (SC-002) | `geo.time.TimeZoneResolverGoldenTest` |
| no-polygon fallback | `geo.time.TimeZoneResolverGoldenTest` (ocean point) |
...
- **instant off by minutes** → a pre-1970 / standard-offset-change row: the
  `tzdb` version moved. Compare `expected.tzdb_version`; regenerate the corpus
  and review the diff.
