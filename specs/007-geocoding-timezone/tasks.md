...
- [x] T001 Parent `pom.xml` — add a `timeshape.version` property (the exact
...
- [x] T002 Extend `tools/ephe-crosscheck/compute_golden.py` with a
...
- [x] T003 Author `geo/src/test/resources/birthmoments/corpus.json` — ≥ 12
...
- [x] T004 [P] Author `geo/src/test/resources/geocode/fixtures.json` (~20
...
- [x] T006 [P] `BirthMomentFlag` enum (`TIME_NOT_KNOWN`, `DST_GAP`, `DST_FOLD`,
  `ZONE_APPROXIMATED`, `POLAR_LATITUDE`, `ZONE_CONFLICT`) in
  `geo/src/main/java/com/celestia/geo/BirthMomentFlag.java`
- [x] T007 [P] `DatasetVersions` record (`String tzdbVersion, String
...
- [x] T009 [P] [US1] `LocalToUtcTest` in
...
- [x] T010 [P] [US1] `TimeZoneResolverGoldenTest` in
...
- [x] T011 [P] [US1] `ZoneResolutionTest` — `ZoneResolution` rejects a null zone
  / blank version

### Implementation

- [x] T012 [US1] `TimeZoneResolver` interface (`ZoneResolution resolve(double
...
- [x] T013 [US1] `TimeshapeTimeZoneResolver` in
...
- [x] T014 [US1] `InstantResolution` record (`Instant instant, ZoneOffset
...
- [x] T015 [P] [US2] `QueryNormalizerTest` + `QueryNormalizerPropertyTest`
...
- [x] T016 [P] [US2] `CoordinateQueryTest` — `"18.52, 73.85"`,
...
- [x] T017 [P] [US2] `CandidateRankingTest` + `CandidateRankingPropertyTest`
...
- [x] T018 [P] [US2] `CachingGeocoderTest` — a counting fake `Geocoder` delegate:
...
- [x] T019 [P] [US2] `FixtureGeocoderTest` (uses the T004 loader) — a known query
  returns its candidates ranked; an unknown query → empty `candidates`, no
  exception

### Implementation

- [x] T020 [P] [US2] `PlaceQuery` (`raw`, `normalized`; `static of(String)`),
...
- [x] T021 [P] [US2] `Geocoder` interface (`GeocodeResult geocode(PlaceQuery)`)
...
- [x] T026 [US2] `FixtureGeocoder` (`FixtureGeocoder(Map<String,
...
- [x] T027 [P] [US3] `ResolvedBirthTest` in
...
- [x] T028 [P] [US3] `BirthMomentResolverTest` in
...
- [x] T029 [P] [US3] `BirthMomentCorpusTest` (`@TestFactory`) in
...
- [x] T032 [P] [US4] `PlaceCandidateTest` — `latitude 95` / `longitude 200` →
  `IllegalArgumentException` (FR-013)
- [x] T033 [P] [US4] `BirthMomentResolverTest` (guardrail cases, extends T028) —
...
- [x] T034 [US4] Confirm the guardrail branches from T031 cover all three cases;
...
- [x] T035 [P] [US5] `GeoDeterminismTest` in
...
- [x] T037 [US5] `.github/workflows/ci.yml` — add `BirthMomentCorpusTest`,
  `TimeZoneResolverGoldenTest`, `GeoDeterminismTest` to the OS-matrix determinism
  step
- [x] T038 [US5] Re-run `python compute_golden.py --birthmoments … ` (no
...
- [x] T039 [P] `geo/src/main/java/com/celestia/geo/REFERENCES.md` (new) — the
  ...
  **cache policy** (`InMemoryGeocodeCache` is permanent for the process
  ...
  (FR-010, FR-018)
- [x] T040 [P] `geo/src/main/java/com/celestia/geo/package-info.java` (update) +
  `place/package-info.java` + `time/package-info.java` — point to SPEC-007, note
  purity and the deferred HTTP adapter
- [x] T041 [P] `BirthMomentResolverPerformanceTest` `@Tag("perf")` in
...
- [x] T042 [P] `geo/README.md` (new) — the module's three capabilities, the
  `Geocoder` port + the deferred OpenCage adapter (ADR-0013), how to regenerate
  the corpus
- [x] T043 Run `specs/007-geocoding-timezone/quickstart.md` end to end; fix drift
- [x] T044 `./mvnw -q verify` — full reactor green incl.
...
- **`timeshape` carries ocean tiles.** `timeshape` 2025b.26 (and
  `timezonefinder.timezone_at`) always return a zone within valid lat/lon —
  open water resolves to a fixed-offset `Etc/GMT±h` tile, not empty. So the
  no-polygon fallback in `TimeshapeTimeZoneResolver` is defensive/unreachable
  with this dataset; `ZONE_APPROXIMATED` is instead raised whenever the resolved
  zone id starts with `Etc/` (an ocean tile is not a civil zone). The tool
  replicates this. `corpus.json` `ocean-pacific` → `Etc/GMT+10` +
  `ZONE_APPROXIMATED`.
- **`CandidateRanking` permutation independence** is scoped: the exact-match
  group is fully ordered by `id` (permutation-independent); the non-match tail
  keeps the provider's order (a stable sort). Run-to-run determinism for a query
  comes from `CachingGeocoder` freezing the first result, not from re-sorting.
  The property test asserts partition + idempotence + exact-group id-ordering.
- **Dataset versions**: `timeshape.version` is filtered into
  `geo/src/main/resources/geo-build.properties` at build time so
  `TimeshapeTimeZoneResolver.DATASET_VERSION` is single-sourced from the pom.
- **`kolkata-1935`** — modern IANA data (Java tzdb 2026a, Python tzdata 2026c)
  both give India +05:30 back past 1941; the pre-1941 +05:53:20 LMT is only in
  `backzone`, which neither loads. The row stays as a pre-1970 case;
  `kolkata-1943` (+06:30) and `london-1944` (+02:00 BDST) are the war-time /
  historical-deviation witnesses both engines agree on.
