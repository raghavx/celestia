...
- **Prerequisite deliverable — DONE**: the golden-chart reference dataset is
  sourced. Three Rodden-AA charts (`einstein-1879`, `jobs-1955`, `obama-1961`) in
  `core/src/test/resources/golden/`, `expected` blocks generated by
  `tools/ephe-crosscheck/compute_golden.py` (pyswisseph + real `.se1` data).
  Remaining polish: T046b (one human lord-chain check / optional 4th textbook
  chart), T046c (confirm Jobs' Rodden rating). Neither blocks `/speckit-implement`.
